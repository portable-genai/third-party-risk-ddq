...
This repository (Rgc8, the Third-Party Risk Due-Diligence Agent) is a **common base** that a bank
...
the Rgc8 artifacts and `domain/models.py` holds this service's request and result types.
...
rewritten. The catalog id `Rgc8` is left alone unless you pass `--catalog-id`, so a fork stays
...
- **Rsk1** compliance assistant: the outsourcing-rule expectation and its citation behind every
  gap, over `CompliancePort` (`adapters/gcp/compliance.py`, an A2A client to Rsk1's regulatory
  ...
  available" reference.
- **Rgc12** contract-obligation register: the vendor's contractual commitments (audit and step-in
  rights, sub-outsourcing, residency, exit assistance, SLAs), over `ContractTermsPort`. Rgc12 is
...
- **Rgc9** consumes the Outsourcing and Material-Arrangements Register this repo writes
...
base (Hrz2). Hrz1 becomes mandatory the moment untrusted free text (a vendor-written DDQ answer,
...
- [ ] Pointed `CompliancePort` at your regulatory KB (or loaded your outsourcing rules into
      Rsk1) and `ContractTermsPort` at your contract register.
