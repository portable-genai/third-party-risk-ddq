...
``TPRM_DDQ_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``TPRM_DDQ_S2S_TOKEN`` is wrong in the most dangerous direction:
...
from .schemas import HealthResponse, TriageRequest, TriageResponse
...
_INSECURE_DEMO_ENV = "TPRM_DDQ_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "TPRM_DDQ_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "TPRM_DDQ_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "TPRM_DDQ_S2S_AUDIENCE"
_API_HOST_ENV = "TPRM_DDQ_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``TPRM_DDQ_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``TPRM_DDQ_PROFILE`` must never be allowed to
    ...
    ``TPRM_DDQ_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "TPRM_DDQ_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "third-party-risk-ddq"
...
_APP_TITLE = "Third-Party Risk Due-Diligence Agent"
...
# names is what answered, never what configuration says would. A request that noted nothing sends
# neither, and the pill keeps showing the configured `generator_model` from `/healthz`.
...
#
# Note what is NOT in this expression: TPRM_DDQ_S2S_TOKEN. A service
...
@app.post("/v1/triage", response_model=TriageResponse, tags=["artifacts"])
def triage(
