...
One env var (``TPRM_DDQ_PROFILE``) selects the adapter family for every
...
_PROFILE_ENV = "TPRM_DDQ_PROFILE"
_SETTINGS_ENV = "TPRM_DDQ_SETTINGS"
...
#: Profiles that mean "running on managed cloud infrastructure", for the model pill's runtime.
...
#: What the offline generator reports itself as: the pill's configured value under ``local`` and
#: the ``X-Answered-By`` the local generator adapter notes when it answers. One constant so the
#: configured pill and the answered pill cannot name two different things for the same stub.
OFFLINE_STUB_MODEL: str = "deterministic-offline-stub"
...
class ProfileChoice:
    ...
    The variable is ``TPRM_DDQ_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("TPRM_DDQ_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "extraction": {
        "local": f"{_PKG}.adapters.local.extraction:LocalExtractionAdapter",
        "gcp": f"{_PKG}.adapters.gcp.extraction:CloudExtractionAdapter",
        "onprem": f"{_PKG}.adapters.onprem.extraction:OnPremExtractionAdapter",
    },
    "contract_terms": {
        "local": f"{_PKG}.adapters.local.contract_terms:LocalContractTermsAdapter",
        "gcp": f"{_PKG}.adapters.gcp.contract_terms:CloudContractTermsAdapter",
        "onprem": f"{_PKG}.adapters.onprem.contract_terms:OnPremContractTermsAdapter",
    },
    "research": {
        "local": f"{_PKG}.adapters.local.research:LocalAdverseMediaAdapter",
        "gcp": f"{_PKG}.adapters.gcp.research:CloudAdverseMediaAdapter",
        "onprem": f"{_PKG}.adapters.onprem.research:OnPremAdverseMediaAdapter",
    },
    "compliance": {
        "local": f"{_PKG}.adapters.local.compliance:LocalComplianceAdapter",
        "gcp": f"{_PKG}.adapters.gcp.compliance:CloudComplianceAdapter",
        "onprem": f"{_PKG}.adapters.onprem.compliance:OnPremComplianceAdapter",
    },
    "register_store": {
        "local": f"{_PKG}.adapters.local.register_store:LocalRegisterStoreAdapter",
        "gcp": f"{_PKG}.adapters.gcp.register_store:CloudRegisterStoreAdapter",
        "onprem": f"{_PKG}.adapters.onprem.register_store:OnPremRegisterStoreAdapter",
    },
    "generation": {
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``TPRM_DDQ_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "DDQ_REVIEW_ROUTING"
...
class Settings:
    ...
    def generator_model(self) -> str:
        ...
        calls: under ``gcp`` the model its call names, never one a flag could swap in.
        ...
        ``no-model`` is deliberately NOT :data:`OFFLINE_STUB_MODEL`. The stub string claims a
...
class Container:
    ...
    @cached_property
    def extraction(self) -> DocumentExtractionPort:
        adapter = self._bind("extraction")
        assert isinstance(adapter, DocumentExtractionPort)
        return adapter

    @cached_property
    def contract_terms(self) -> ContractTermsPort:
        adapter = self._bind("contract_terms")
        assert isinstance(adapter, ContractTermsPort)
        return adapter

    @cached_property
    def research(self) -> AdverseMediaPort:
        adapter = self._bind("research")
        assert isinstance(adapter, AdverseMediaPort)
        return adapter

    @cached_property
    def compliance(self) -> CompliancePort:
        adapter = self._bind("compliance")
        assert isinstance(adapter, CompliancePort)
        return adapter

    @cached_property
    def register_store(self) -> RegisterStorePort:
        adapter = self._bind("register_store")
        assert isinstance(adapter, RegisterStorePort)
        return adapter
