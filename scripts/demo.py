...
from tprm_ddq.domain import (
    kernel,
    models,
)
from tprm_ddq.domain.pii import (
    JURISDICTIONS,
)
...
SERVICE_NAME = "Third-Party Risk Due-Diligence Agent"
CATALOG_ID = "third-party-risk-ddq"
REPOSITORY = "third-party-risk-ddq"
...
def _exit_extraction(container: Any) -> Any:
    from tprm_ddq.domain.tprm_models import DocumentRef

    return container.extraction.extract(
        DocumentRef(doc_id="ev", content="CONTROL soc2 CC6.1 effective 2025-01-01", mime_type="t")
    )


def _exit_contract_terms(container: Any) -> Any:
    return container.contract_terms.commitments("Vendor (FICTIONAL)", tenant=TENANT)


def _exit_research(container: Any) -> Any:
    return container.research.search("Vendor (FICTIONAL)")


def _exit_compliance(container: Any) -> Any:
    from tprm_ddq.domain.tprm_models import CanonicalControl

    return container.compliance.requirements((CanonicalControl.ACCESS_CONTROL,), actor=ACTOR)


def _exit_register_store(container: Any) -> Any:
    from datetime import date

    from tprm_ddq.domain.tprm_models import RegisterEntry

    return container.register_store.upsert(
        RegisterEntry(
            vendor="Vendor (FICTIONAL)",
            tenant=TENANT,
            residual_band=kernel.Severity.HIGH,
            material=False,
            as_of=date(2026, 1, 1),
        )
    )
...
def _exit_generation(container: Any) -> Any:
    return container.generation.draft_memo("Vendor (FICTIONAL)", "high", ("gap-1",))
...
EXIT_CALLS: dict[str, Callable[[Any], Any]] = {
    ...
    "contract_terms": _exit_contract_terms,
    "research": _exit_research,
    "compliance": _exit_compliance,
    "register_store": _exit_register_store,
