...
The catalog id (``Rgc8``) is left alone unless ``--catalog-id`` is given: it names the entry in
...
_OLD_PACKAGE = "tprm_ddq"
_OLD_ENV_PREFIX = "TPRM_DDQ_"
_OLD_BARE_PREFIX = "TPRM_DDQ"
_OLD_RESOURCE = "rgc8-svc"
_OLD_DIST = "third-party-risk-ddq"
_OLD_CATALOG_ID = "Rgc8"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    # The env prefix, only where it PREFIXES an upper token (TPRM_DDQ_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource",
        required=True,
        help="new cloud resource stem (Terraform name_prefix), e.g. acme-tprm",
    )
