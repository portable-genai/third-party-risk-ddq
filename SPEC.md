# SPEC: Third-Party Risk Due-Diligence Agent (`third-party-risk-ddq`)
...
- **Maker-checker (P-06) and routing (R8)**: a HIGH/CRITICAL result sets
  ...
  logged rather than failing the result. `DDQ_REVIEW_ROUTING` switches routing (default on).
- **Profile**: resolved ONCE, at import, into a `ProfileChoice` and never a bare string. Three
  states of `TPRM_DDQ_PROFILE`: UNSET is NO CHOICE (the SDK-free adapters
