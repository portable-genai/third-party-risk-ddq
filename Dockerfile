...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    TPRM_DDQ_PROFILE=gcp \
    TPRM_DDQ_SETTINGS=/app/config/settings.yaml \
    PORT=8080
...
CMD ["sh", "-c", "python -m tprm_ddq.managed_readiness && exec uvicorn tprm_ddq.api.app:app --host 0.0.0.0 --port ${PORT:-8080}"]
