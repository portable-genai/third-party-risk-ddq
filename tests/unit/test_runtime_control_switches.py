...
control this service has: ``DDQ_REVIEW_ROUTING`` is read in three states; off binds a disabled
router and says so at startup; on under the managed profile refuses to boot without a console;
and the API, the agent tool and the CLI report ``review_routing`` rather than failing an
already-scored, already-audited result when the console is unreachable.
"""
...
from tprm_ddq.adapters.controls import (
    DisabledReviewRouter,
    RecordingReviewRouter,
    ReviewRouting,
)
from tprm_ddq.agent import tools
from tprm_ddq.api import app as api_module
from tprm_ddq.api.app import app
from tprm_ddq.cli.main import main as cli_main
...
from tprm_ddq.envread import ConfiguredEmptyError
...
_LOCAL_ROUTE = "tprm_ddq.adapters.local.review_router.LocalReviewRouter.route"
...
def _result(text: str = "urgent data breach") -> TriageResult:
    ...
    return service.triage(TriageInput("Acme Holdings (FICTIONAL)", text), actor="a@bank.example")


def _gcp(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "tprm_ddq.config.resolve_profile", lambda environ=None: ProfileChoice("gcp", True)
    )
...
def test_the_off_posture_is_logged_once_however_many_containers(
    ...
    settings = _settings(controls=ControlSwitches(review_routing=False))
    with caplog.at_level(logging.WARNING, logger="tprm_ddq.config"):
        for _ in range(3):
...
def test_a_failed_hand_off_is_reported_and_logged_never_raised(
    ...
    with caplog.at_level(logging.WARNING, logger="tprm_ddq.adapters.controls"):
...
def test_the_agent_tool_reports_the_hand_off() -> None:
    payload = tools.triage_case(**_ESCALATING, tenant="demo-bank", settings=_settings())
    assert payload["review_routing"] == "routed"


def test_the_agent_tool_reports_routing_off() -> None:
    settings = _settings(controls=ControlSwitches(review_routing=False))
    payload = tools.triage_case(**_ESCALATING, tenant="demo-bank", settings=settings)
...
def test_the_cli_reports_the_hand_off(capsys: pytest.CaptureFixture[str]) -> None:
    assert cli_main(["triage", _ESCALATING["subject"], _ESCALATING["text"]]) == 0
