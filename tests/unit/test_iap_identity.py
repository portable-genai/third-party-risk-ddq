...
from tprm_ddq.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from tprm_ddq.ports.identity import (
    VERIFIED,
...
_PKG = "tprm_ddq"
#: Every profile, so the domain ports outside the triage path bind all three in the fixture.
_ALL_PROFILES = ("local", "gcp", "onprem")
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        # The domain ports beyond the triage path are bound to their local adapters here: this
        # test exercises identity on /v1/triage, and the exact-port-set loader still requires
        # every declared port to appear. Real deployments bind these to gcp/onprem in settings.
        "  extraction:",
        *[
            f"    {p}: {_PKG}.adapters.local.extraction:LocalExtractionAdapter"
            for p in _ALL_PROFILES
        ],
        "  contract_terms:",
        *[
            f"    {p}: {_PKG}.adapters.local.contract_terms:LocalContractTermsAdapter"
            for p in _ALL_PROFILES
        ],
        "  research:",
        *[
            f"    {p}: {_PKG}.adapters.local.research:LocalAdverseMediaAdapter"
            for p in _ALL_PROFILES
        ],
        "  compliance:",
        *[
            f"    {p}: {_PKG}.adapters.local.compliance:LocalComplianceAdapter"
            for p in _ALL_PROFILES
        ],
        "  register_store:",
        *[
            f"    {p}: {_PKG}.adapters.local.register_store:LocalRegisterStoreAdapter"
            for p in _ALL_PROFILES
        ],
        "  generation:",
        *[
